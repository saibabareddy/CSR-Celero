...
//                            $(newSelect).find('option:selected').next('option');
//                            $('option:selected').next('option')
//                            $('option:selected',newSelect).next('option').val();
                            
                            $(newSelect).val(function(){
    ...
    return $(nextOption).val();
                       ...
                       newSelectValue=$(newRef).find('select').val();
//                        i++;
                        
//                       $(newSelect).find('[value="'+c+'"]').attr("selected",null);
//       ******************  $(newSelect).find('[value="'+c+'"]').remove();
                    });

//                      $(this).find('[value="X"]').remove();
                      
//        ***********         $(newSelect).find('[value="RED"]').remove();


//                      $(newSelect).find('[value="RED"]').prop("selected", false);           
//   *************     $(newSelect).trigger("change");
//                     newSelect.remove('option','RED');
                    
                    var x=$('tr').has('.' + style);
                    
                   (( $('tr').has('.' + style)).not(newRef)).each(function() {
                     var color = $(this).find("select").val();   
                 });
//                    colorSelection(newSelect,0);
                }
//                if (variant_size === 1) {
//                 element.closest('tr').find('.btn').prop('disabled', true);
//                }

//                 var newRef=($(currentRow).closest('tr')).has('.'+style);
...
//                      var items=[ {"style":"DRESS1","colors":colors,} ];          
    ...
    var rowNew = $(' <tr>' +
            ...
            '<td class="mainTable" style="vertical-align:middle"><select name="COLOR" onChange="colorSelection(this,'+currentIndex+')" class="form-control" style="width:210px">' +
//                                    
//                                        '<option>GREEN</option>'+
//                                        '<option>BLUE</option>'+
...
//                               var option = new Option(text, value); $('select').append($(option));
    ...
    for (i = 0; i < sizes.length; i++) {
        $(sizeRow).append('<td style="width:50px;color:gray;font-weight:bold;">' + sizes[i] + '</td>');
    }
...
//  --------------------------------------------------------------------
 ...
 var row3='<th style="color:gray">Price</th>';
  ...
  row1=row1+'<td style="width:50px;color:gray;font-weight:bold;">'+sizes[i]+'</td>';
  }
 ...
 for(i=0;i<sizes.length;i++){
    row3=row3+'<td><input type="hidden"  name="" value="'+prices[i]+'"><p>'+prices[i]+'</p></td>';
                               } 
